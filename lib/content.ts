...
import { blogPosts, fieldCases, inquiries, type NewBlogPost, type NewFieldCase } from "@/lib/db/schema";
...
export const fallbackPosts = [
  ...
  },
  {
    ...
    content:
      "출강 전에는 교육 목적, 참여자 수준, 현장에서 사용할 도구 환경을 확인합니다.\n\n출강 후에는 수료 여부만 남기지 않습니다. 실습 결과물, 현장 질문, 다음 교육 제안까지 정리해 기관이 후속 교육을 판단할 수 있게 합니다.",
    published: true,
    featured: false,
  ...
  },
  {
    ...
    content:
      "실습형 교육에는 도구 목록보다 재현 가능한 문서가 필요합니다.\n\n프롬프트 템플릿, 입력 예시, 결과물 검토 기준을 함께 제공해야 교육 후에도 반복해서 사용할 수 있습니다.",
    published: true,
    featured: false,
...
export async function getPostBySlug(slug: string) {
  if (!hasDatabase) return fallbackPosts.find((post) => post.slug === slug) ?? null;
...
export async function getCaseBySlug(slug: string) {
  if (!hasDatabase) return fallbackCases.find((item) => item.slug === slug) ?? null;
